...
GC create_gc (Display* display, int screen_num, Window win, int type)
{
  ...
  switch (type) {
      case 1:
        ...
        XSetLineAttributes(display, gc, FRAME_W, line_style, cap_style, join_style);
        break;
      case 0:
...
if (!d) { fprintf (stderr, "can't draw frame\n"); }
}
...
int p; for (p=0; p<NCOLORS; p++) {
    if (g.color[p].name[0]) {
        ...
        if (!XAllocNamedColor (dpy, 
                    ...
                    &(g.color[p].xcolor), 
                    &(g.color[p].xcolor)) )
            die2 ("failed to allocate X color: ", g.color[p].name);
        if (!XftColorAllocName (dpy,
                    visual,
                    colormap,
                    g.color[p].name,
                    &(g.color[p].xftcolor)) )
            die2 ("failed to allocate Xft color: ", g.color[p].name);
    }
}

if (g.debug>0) {fprintf (stderr, "early opening font\n");}
//fontLabel = XLoadFont (dpy, LABELFONT);  // using Xft instead
fontLabel = XftFontOpenName (dpy, scrNum, g.option_font);
if (!fontLabel) {fprintf (stderr, "can't allocate font: %s", g.option_font);}

// having colors, GC may be built
// they are required early for addWindow when transforming icon depth
if (g.debug>0) {fprintf (stderr, "early building GCs\n");}
...
{
if (g.debug>0) {fprintf (stderr, "preparing ui\n");}
...
if (!initWinlist (dpy, root, direction)) {
    if (g.debug>0) {fprintf (stderr, "initWinlist failed, skipping ui initialization\n");}
    g.winlist=NULL; g.maxNdx=0;
    return 0;
}
if (!g.winlist) {
    if (g.debug>0) {fprintf (stderr, "winlist doesn't exist, skipping ui initialization\n");}
    return 0;
}
if (g.maxNdx<1) {
    if (g.debug>0) {fprintf (stderr, "number of windows < 1, skipping ui initialization\n");}
    return 0;
}
if (g.debug>0) {
...
if (g.debug>0) {fprintf (stderr, "tile w=%d h=%d win w=%d h=%d\n", tileW, tileH, uiwinW, uiwinH);}
...
int m; for (m=0; m<g.maxNdx; m++) {
    ...
    if (!fr) { fprintf (stderr, "can't fill tile\n"); }
    // place icons
    if (g.winlist[m].icon_drawable) {
        if (g.winlist[m].icon_w == iconW && 
            ...
            if (g.debug>1) {fprintf (stderr, "%d: copying icon\n", m);}
            int or = XCopyArea (dpy, 
                    g.winlist[m].icon_drawable, g.winlist[m].tile, 
                    g.gcDirect, 0, 0, 
                    g.winlist[m].icon_w, g.winlist[m].icon_h,  // src
                    0, 0);  // dst
            if (!or) {fprintf (stderr, "can't copy icon to tile\n");}
        } else {
            // scale
            if (g.debug>1) {fprintf (stderr, "%d: scaling icon\n", m);}
            int sc = pixmapScale (dpy, scrNum, root, 
                    g.winlist[m].icon_drawable, g.winlist[m].tile,
                    g.winlist[m].icon_w, g.winlist[m].icon_h,
                    iconW, iconH);
            if (!sc) {fprintf (stderr, "can't scale icon to tile\n");}
        }
    } else {
        ...
        if (!gcL) {
            ...
            int pr = XDrawRectangle (dpy, g.winlist[m].tile, gcL, 0, 0, iconW, iconH);
            if (!pr) {fprintf (stderr, "can't draw placeholder\n");}
            XFreeGC (dpy, gcL);
        }
    }
    // draw labels
    if (g.winlist[m].name && fontLabel) {
        int dr = drawMultiLine (dpy, g.winlist[m].tile, fontLabel,
                ...
                g.winlist[m].name, 
                0, (iconH+5), tileW, (tileH-iconH-5));
        if (dr!=1) {fprintf (stderr, "can't draw label\n");}
    }
}
if (g.debug>0) {fprintf (stderr, "prepared %d tiles\n", m);}
...
if (at && wt && td)
    XChangeProperty (dpy, uiwin, wt, at, 32, PropModeReplace, (unsigned char*)(&td), 1);
...
// icons
int j; for (j=0; j<g.maxNdx; j++) {
    if (g.winlist[j].tile) {
        if (g.debug>1) {fprintf (stderr, "copying tile %d to canvas\n", j);}
        //XSync (dpy, false);
        int r=XCopyArea (dpy, g.winlist[j].tile, uiwin, 
                g.gcDirect, 0, 0, tileW, tileH,  // src
                j*(tileW+FRAME_W)+FRAME_W, FRAME_W);  // dst
        //XSync (dpy, false);
        if (g.debug>1) {fprintf (stderr, "XCopyArea returned %d\n", r);}
    }
}
// frame
framesRedraw (dpy);
...
{
if (!uiwin) return 0; // kb events may trigger it even when no window drawn yet
g.selNdx++;
if (g.selNdx >= g.maxNdx)
    g.selNdx=0;
if (g.debug>0) {fprintf (stderr, "item %d\n", g.selNdx);}
framesRedraw (dpy);
return 1;
}
