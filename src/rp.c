...
if (strlen(ratpoison_cmd) <2 ) {
    ...
    if (g.debug>0) {fprintf (stderr, "ratpoison: %s\n", ratpoison_cmd);}
}
...
if (!execAndReadStdout (ratpoison_cmd, ua, buf, MAXRPOUT)) {
    fprintf (stderr, "can't get unmanaged window list from ratpoison\n");
    return 1; // not fatal
}
if (g.debug>1) {fprintf (stderr, "ratpoison reports unmanaged:\n%s", buf);}
if (!strstr(buf, XWINNAME)) {
    if (g.debug>0) {fprintf (stderr, "registering in ratpoison unmanaged list\n");}
    //memset (buf, '\0', MAXRPOUT);
    if (!execAndReadStdout (ratpoison_cmd, uains, buf, MAXRPOUT)) {
        fprintf (stderr, "can't register in ratpoison unmanaged window list\n");
        return 1; // not fatal
    }
} else {
    if (g.debug>0) {fprintf (stderr, "our window is already in ratpoison unmanaged list\n");}
}
...
if (strstr (buf, "No managed windows")) {
    ...
    while ((tok=strsep (&rest, "\r\n")) && (*tok)) {
        if (g.debug>1) {fprintf (stderr, "token: %s\n", tok);}

        // parse single line
        char* rest2=tok; char* tok2; char* endptr;
